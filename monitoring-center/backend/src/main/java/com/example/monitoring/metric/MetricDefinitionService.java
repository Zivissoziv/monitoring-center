...
public class MetricDefinitionService {
    ...
    @Autowired
    private com.example.monitoring.agent.AgentRepository agentRepository;
    ...
    private void initializeConfigsForAllAgents(String metricName) {
        List<com.example.monitoring.agent.Agent> agents = agentRepository.findAll();
        
        for (com.example.monitoring.agent.Agent agent : agents) {
            // Check if config already exists
            java.util.Optional<com.example.monitoring.metric.AgentMetricConfig> existing = 
                agentMetricConfigRepository.findByAgentIdAndMetricName(agent.getId(), metricName);
            
            if (!existing.isPresent()) {
                // Create new config
                com.example.monitoring.metric.AgentMetricConfig config = 
                    new com.example.monitoring.metric.AgentMetricConfig(agent.getId(), metricName);
